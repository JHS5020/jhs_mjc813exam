...
public class CinemaRestController {
    ...
    private ResponseDto getResponseDto( String msg, Integer resultCode, Object data) {
    ...
    public ResponseEntity<ResponseDto> update(@RequestBody CinemaDto dto) {
        try {
            CinemaDto result = this.cinemaService.updateCinema(dto);
            return ResponseEntity.ok().body(this.getResponseDto("성공",30, result));
        } catch (Exception e) {
            log.error(e.toString());
            return ResponseEntity.ok().body(this.getResponseDto("실패", 0, null));
        }
    }
    ...
    public ResponseEntity<ResponseDto> delete(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.ok().body(this.getResponseDto("성공",30, true)
            );
        } catch (Exception e) {
            log.error(e.toString());
            return ResponseEntity.ok().body(this.getResponseDto("실패", 0, null)
